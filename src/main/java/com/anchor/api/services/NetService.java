...
public class NetService {
    ...
    public String sendOzowPaymentRequest(OzowPaymentRequest request) throws Exception {
        LOGGER.info(E.PEAR.concat(E.PEAR).concat(E.PEAR).concat("Sending ozow payment request ... ... ... check fields: \uD83D\uDD35 "
                ));

        String hash = hashCheckGenerator.generateOzowHash(request);

        LOGGER.info(E.BLUE_DOT.concat(E.BLUE_DOT).concat("URL for request: ".concat(ozowUrl)));
...
//        }
        ...
        LOGGER.info(E.CAT.concat(E.CAT.concat(E.CAT.concat(E.CAT)).concat("Parameters sent with request")));
        for (NameValuePair pair : pairs) {
            LOGGER.info(E.CAT.concat(E.CAT
                    .concat("Parameter: ")
...
//        httpPost.addHeader("Content-Type", "application/json");
        ...
        try (CloseableHttpResponse response2 = httpclient.execute(httpPost)) {
            LOGGER.info(E.HEART_BLUE.concat(E.HEART_BLUE.concat(E.HEART_PURPLE))
                    .concat("Response StatusLine: ".concat(String.valueOf(response2.getStatusLine()))));
            if (response2.getStatusLine().getStatusCode() == 200) {
                LOGGER.info(E.HAND2.concat(E.HAND2).concat("Ozow responded with a SUCCESS status code = 200 "
                        .concat(E.RED_APPLE).concat(response2.getStatusLine().getReasonPhrase()).concat(E.BLUE_DOT)));
            }
            if (response2.getStatusLine().getStatusCode() == 302) {
                LOGGER.info(E.HAND2.concat(E.HAND2).concat("Ozow responded with a RE_DIRECTION status code = 302 "
                        .concat(E.RED_APPLE).concat(response2.getStatusLine().getReasonPhrase()).concat(E.BLUE_DOT)));
            }

            LOGGER.info(E.BROCCOLI.concat(E.BROCCOLI).concat(("ozow payment request; " +
                    "\uD83C\uDF3F response is: ").concat(response2.toString())));

            HttpEntity entity2 = response2.getEntity();
            final String entityString = EntityUtils.toString(entity2);
            LOGGER.info(E.LEAF.concat(E.LEAF).concat("entityString extracted: ".concat(entityString)));
